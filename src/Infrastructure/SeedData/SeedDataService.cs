...
public class SeedDataService
{
    ...
    private async Task SeedPaymentsAsync()
    {
        ...
        var paymentFaker = new Faker<Payment>()
            ...
            .RuleFor(p => p.TransactionId, f => $"TXN_{f.Random.AlphaNumeric(16)}")
            .RuleFor(p => p.ProviderTransactionId, f => $"PROV_{f.Random.AlphaNumeric(16)}")
            .RuleFor(p => p.PaymentDate, f => f.Date.Past(6))
            .RuleFor(p => p.Currency, "TRY")
            .RuleFor(p => p.Description, f => f.Random.Bool(0.7f) ? f.Lorem.Sentence() : null)
            .RuleFor(p => p.RefundAmount, f => f.Random.Bool(0.1f) ? f.Random.Decimal(100, 1000) : null)
            .RuleFor(p => p.RefundDate, f => f.Random.Bool(0.1f) ? f.Date.Past(3) : null)
            .RuleFor(p => p.RefundReason, f => f.Random.Bool(0.1f) ? f.Lorem.Sentence() : null)
            .RuleFor(p => p.ErrorMessage, f => f.Random.Bool(0.05f) ? f.Lorem.Sentence() : null)
            .RuleFor(p => p.RetryCount, f => f.Random.Int(0, 3))
            .RuleFor(p => p.LastRetryDate, f => f.Random.Bool(0.1f) ? f.Date.Past(1) : null)
            .RuleFor(p => p.LastFourDigits, f => f.Random.Bool(0.8f) ? f.Random.Int(1000, 9999).ToString() : null)
            .RuleFor(p => p.CardType, f => f.Random.Bool(0.8f) ? f.PickRandom("Visa", "MasterCard", "American Express") : null)
            .RuleFor(p => p.ExchangeRate, f => f.Random.Bool(0.2f) ? f.Random.Decimal(1, 30) : null)
            .RuleFor(p => p.OriginalAmount, f => f.Random.Bool(0.2f) ? f.Random.Decimal(50, 500) : null)
            .RuleFor(p => p.OriginalCurrency, f => f.Random.Bool(0.2f) ? f.PickRandom("USD", "EUR", "GBP") : null)
