...
/// <summary>
/// Ev Ekleme DTO Validatörü
/// </summary>
public class AddPropertyDtoValidator : AbstractValidator<AddPropertyDto>
{
    public AddPropertyDtoValidator()
    {
        RuleFor(x => x.HostId)
            .NotEmpty().WithMessage("Ev sahibi ID boş olamaz.");

        RuleFor(x => x.Title)
            .NotEmpty().WithMessage("Ev başlığı boş olamaz.")
            .Length(5, 200).WithMessage("Ev başlığı 5-200 karakter arasında olmalıdır.");

        RuleFor(x => x.Description)
            .NotEmpty().WithMessage("Ev açıklaması boş olamaz.")
